...
try:
    client.collections["Tillich-Lectures"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "Tillich-Lectures",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "Tillich-Lectures",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/TillichCorrespondence/tillich-lectures-static/{record['id']}"
...
print(make_index)
print("done with indexing Tillich-Lectures")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index Tillich-Lectures")
